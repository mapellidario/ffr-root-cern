/*
g++ -o fft_example fft_example.cpp `root-config --cflags --glibs`
*/
...
/*	//nSamples of f(x) arbitraria: 
	TH1D h1dInit("h1dInit", "Sawtooth Wave", nSamples, 0, 10.*twoPi);
	for (Int_t bin = 1; bin <= nSamples; bin++){
		Double_t x = h1dInit.GetBinLowEdge(bin);
		Double_t value = sin(x)+sin(x/2.)/2.;
		h1dInit.SetBinContent(bin , value);
	}
*/	
/*	
	// nSamples samples of f(x) = white noise w/ rms = RMS
...
*/	
	...
	c5->cd();
	h1dInitProj.Draw();
	//questo istogramma è ripempito con il conentuo d ogni singolo bin dell'istogramma precedente
	//quindi riporta la frequenza con cui ricorrono le immagini all'interno del codominio della 
	//funzione f(x)
	// initiate the FFT
